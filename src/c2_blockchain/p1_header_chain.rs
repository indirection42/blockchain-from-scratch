...
impl Header {
	...
	fn verify_sub_chain(&self, chain: &[Header]) -> bool {
		if chain.len() == 1 {
			...
			if chain[0].height == 1 && chain[0].parent != hash(&Header::genesis()) {
				return false;
			}
		}
		for window in chain.windows(2) {
			let parent = &window[0];
			let child = &window[1];
			if child.parent != hash(&parent) || child.height != parent.height + 1 {
				return false;
			}
		}
		true
...
fn build_valid_chain_length_5() -> Vec<Header> {
	...
	for _ in 0..5 {
		chain.push(chain.last().map_or_else(Header::genesis, |h: &Header| h.child()));
	}
	chain
}
